<script lang="ts">
	...
	const amountIncrementError = $derived(
		payment === "dp" &&
			...
			(!Number.isInteger(effectiveAmount) || effectiveAmount % PAYMENT_AMOUNT_STEP !== 0),
	);
	...
	const loadPaymentConfigs = async () => {
		try {
			...
			paymentError =
				e instanceof Error ? e.message : "Gagal memuat metode pembayaran.";
		}
	};
	...
	const phoneError = $derived(
		phone.length > 0 && (!validLocalPhone || !isValidPhone(phoneWithPrefix)),
	);
...
{#if submitted}
	<div class="w-full rounded-xl border border-border/60 bg-background/60 p-4 text-center" role="status">
		...
		<div class="mt-4 rounded-xl border border-cyan-300/30 bg-cyan-300/10 p-4 text-left" data-testid="registration-status-banner">
			<p class="text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-200">Status pendaftaran</p>
			<p class="mt-1 font-display text-lg font-extrabold uppercase text-white">{successStatus}</p>
			<p class="mt-1 text-xs leading-relaxed text-slate-300">{successStatusDescription}</p>
		</div>
		{#if submitted.ticketNumber}
			<div class="mt-4 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4">
				<p class="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-200">Nomor tiket</p>
				<p class="mt-1 text-2xl font-bold tabular-nums text-white">{submitted.ticketNumber}</p>
				<p class="mt-1 text-xs text-amber-100/80">Simpan nomor ini sebagai cadangan saat check-in.</p>
			</div>
		{/if}

		{#if submitted.participantId}
			<div class="mt-4 rounded-xl border border-indigo-300/25 bg-indigo-300/10 p-4">
				<div class="flex items-center justify-center gap-2 text-left text-sm font-bold text-indigo-100">
					<QrCode class="h-4 w-4 shrink-0" aria-hidden="true" />
					QR check-in panitia
				</div>
				<div class="mx-auto mt-3 w-fit rounded-xl bg-white p-2" data-testid="registration-qr">
					<QRCode id={submitted.participantId} size={176} />
				</div>
				<p class="mt-3 text-xs leading-relaxed text-indigo-100/80">Tunjukkan QR ini kepada panitia. QR terhubung langsung ke data peserta dan bisa dipindai dari menu Check-in.</p>
			</div>
		{:else if submitted.queued}
			<div class="mt-4 rounded-xl border border-amber-300/25 bg-amber-300/10 p-4 text-left text-xs leading-relaxed text-amber-100">QR akan muncul setelah data berhasil tersinkronisasi. Untuk sementara, simpan nomor WA dan detail pendaftaran ini.</div>
		{/if}

		<div class="mt-4 grid gap-3 text-left sm:grid-cols-2">
			<div class="rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-4">
				<div class="flex items-center gap-2 text-sm font-bold text-cyan-100"><LogIn class="h-4 w-4" aria-hidden="true" />Cara login lagi</div>
				<p class="mt-2 text-xs leading-relaxed text-slate-300">Tekan tombol Login, lalu masukkan nomor WA yang sama. Profil pendaftar dan semua tiket akan tampil kembali.</p>
			</div>
			<div class="rounded-xl border border-emerald-300/20 bg-emerald-300/5 p-4">
				<div class="flex items-center gap-2 text-sm font-bold text-emerald-100"><Camera class="h-4 w-4" aria-hidden="true" />Screenshot / simpan</div>
				<p class="mt-2 text-xs leading-relaxed text-slate-300">Screenshot halaman ini atau simpan nomor tiket dan QR sebelum menutup halaman.</p>
			</div>
		</div>
				...
				{:else}
					...
					{:else}
						...
						<p class="text-[11px] leading-relaxed text-muted-foreground">
							Terisi otomatis dari card lomba. DP memakai kelipatan Rp 500 (Rp500, Rp1.000, Rp5.000, Rp10.000, Rp20.000, Rp50.000, Rp100.000).
						</p>
		...
		<div class="mt-5 flex flex-col justify-center gap-2 sm:flex-row">
			{#if onLogin}
				<button type="button" class="btn btn-gold" onclick={() => onLogin?.()}>
					<LogIn class="h-4 w-4" aria-hidden="true" />
					Lanjut login
				</button>
			{/if}
			<button type="button" class="btn btn-ghost" onclick={() => (submitted = null)}>
...
{:else}
	...
	>
		...
		<label class="flex flex-col gap-1 text-sm">
			...
			<div class="flex min-w-0 items-center rounded-lg border border-border bg-background focus-within:ring-2 focus-within:ring-gold/60">
				<span class="shrink-0 border-r border-border px-2 py-2.5 text-sm font-semibold text-muted-foreground" aria-hidden="true">+62</span>
				<input
					type="tel"
					value={phone}
					oninput={(event) => {
						phone = (event.currentTarget as HTMLInputElement).value.replace(/\D/g, "");
					}}
			...
			{#if phoneError}
				<span class="text-xs text-destructive">Nomor WA harus diawali angka 8 setelah prefix +62.</span>
			{/if}
		</label>

		<fieldset class="flex min-w-0 flex-col gap-2 text-sm">
			<legend class="font-medium">Pilih lomba</legend>
			<div class="no-scrollbar mt-2 flex min-w-0 w-full max-w-full snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain pb-2 [touch-action:pan-x]" role="radiogroup" aria-label="Pilihan lomba">
				{#each competitions as c (c.id)}
					{@const selected = competitionId === c.id}
					{@const prizes = prizesFor(c)}
					<div class="relative flex w-[16rem] shrink-0 snap-start flex-col overflow-hidden rounded-2xl border {selected ? 'border-cyan-300/70 shadow-[0_0_24px_rgba(34,211,238,0.14)]' : c.isActive ? 'border-slate-700' : 'border-rose-200/45 shadow-[0_0_24px_rgba(220,38,38,0.14)]'} {c.isActive ? 'bg-[#0a0f1c]' : 'bg-[linear-gradient(135deg,rgba(127,29,29,0.72),rgba(10,15,28,0.96)_58%,rgba(248,250,252,0.1))]'}">
						<input
							id={`competition-${c.id}`}
							type="radio"
							name="competition"
							...
							class="sr-only"
						/>
						<label for={`competition-${c.id}`} class="flex flex-1 cursor-pointer flex-col gap-3 p-4 {c.isActive ? '' : 'cursor-not-allowed'}">
							<div class="flex items-start justify-between gap-2">
								<span class="font-display text-[10px] font-bold uppercase tracking-widest {c.isActive ? 'text-cyan-300' : 'text-rose-100'}">
									{c.isActive ? "Siap Bertanding" : "Segera Dimulai"}
								</span>
								<span class="font-display text-xs text-slate-500">{c.totalQuota} slot</span>
							</div>
							<h3 class="font-display break-words text-lg font-extrabold uppercase leading-tight text-slate-100">{c.name}</h3>
							<div class="grid grid-cols-2 gap-2 rounded-xl border border-slate-800 bg-slate-950/40 p-3 text-xs">
								<div>
									<p class="font-bold uppercase tracking-wider text-slate-500">Tiket</p>
									<p class="mt-1 break-words font-semibold text-slate-100">Rp {c.fee.toLocaleString("id-ID")}</p>
								</div>
								<div class="border-l border-slate-800 pl-2">
									<p class="font-bold uppercase tracking-wider text-slate-500">DP mulai</p>
									<p class="mt-1 break-words font-semibold text-slate-100">Rp {c.minDp.toLocaleString("id-ID")}</p>
								</div>
							</div>
							<div class="rounded-xl border border-indigo-300/15 bg-indigo-300/5 p-3">
								<p class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-indigo-200">
									<Gift class="h-3.5 w-3.5" aria-hidden="true" />
									Hadiah-hadiah
								</p>
								<ul class="mt-2 list-disc space-y-1 pl-5 text-xs text-slate-300">
									{#each prizes.slice(0, 3) as prize}
										<li class="break-words pl-1">{prize}</li>
									{/each}
								</ul>
							</div>
						</label>
			...
			{#if selectedCompetition}
				<p class="text-xs text-cyan-200">Terpilih: {selectedCompetition.name}</p>
			{:else}
				<p class="text-xs text-slate-500">Geser kartu lalu pilih arena yang ingin diikuti.</p>
			{/if}
		</fieldset>
		...
		<div class="relative overflow-hidden rounded-xl border border-rose-200/35 bg-[linear-gradient(110deg,rgba(127,29,29,0.92),rgba(15,23,42,0.96)_62%,rgba(248,250,252,0.1))] p-4 text-rose-50 shadow-[0_0_24px_rgba(220,38,38,0.12)]" role="note">
			<div class="absolute inset-y-0 left-0 w-1 bg-white" aria-hidden="true"></div>
			<div class="flex gap-2 pl-2">
				<AlertTriangle class="mt-0.5 h-4 w-4 shrink-0 text-white" aria-hidden="true" />
				<div>
					<p class="font-display text-xs font-bold uppercase tracking-wider text-white">No-refund</p>
					<p class="mt-1 text-xs leading-relaxed text-rose-100">Biaya pendaftaran yang sudah dibayar tidak dapat dikembalikan. Pastikan pilihan lomba dan data peserta sudah benar.</p>
				</div>
			</div>
		</div>
...
{#if termsCompetition}
		<div class="fixed inset-0 z-50 flex overscroll-none items-center justify-center overflow-hidden bg-slate-950/80 px-2 py-4 backdrop-blur-sm">
		<div
			...
			role="dialog"
			aria-modal="true"
			aria-labelledby="competition-terms-title"
		>
			<div class="flex items-start justify-between gap-3 border-b border-slate-800 p-4">
				<div>
					<p class="text-[10px] font-bold uppercase tracking-widest text-cyan-300">Ketentuan lomba</p>
					<h2 id="competition-terms-title" class="font-display mt-1 break-words text-xl font-extrabold uppercase text-slate-100">{termsCompetition.name}</h2>
				</div>
				<button type="button" class="btn btn-ghost btn-sm shrink-0" aria-label="Tutup ketentuan" onclick={() => (termsCompetition = null)}>
					<X class="h-4 w-4" aria-hidden="true" />
				</button>
			</div>
				<div class="min-h-0 overflow-y-auto overscroll-contain p-4 [touch-action:pan-y]">
				<div class="rounded-xl border border-indigo-300/15 bg-indigo-300/5 p-3">
					<p class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-indigo-200">
						<Gift class="h-3.5 w-3.5" aria-hidden="true" />
						Hadiah
					...
					<ul class="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-200">
						{#each prizesFor(termsCompetition) as prize}
							<li class="pl-1">{prize}</li>
						{/each}
					</ul>
				</div>
				<h3 class="font-display mt-5 text-sm font-bold uppercase tracking-wider text-cyan-200">Ketentuan & persyaratan</h3>
					<ol class="mt-2 list-decimal space-y-2 pl-5 text-sm leading-relaxed text-slate-300">
						{#each termsFor(termsCompetition) as term}
							<li class="pl-1">{term}</li>
					{/each}
				</ol>
			</div>
		</div>
