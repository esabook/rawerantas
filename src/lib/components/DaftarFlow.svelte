<script lang="ts">
	...
	import { LogIn, UserRound, X } from "@lucide/svelte";
	...
	let { lockedSlug }: { lockedSlug?: string } = $props();
	...
	const lockedCompetition = $derived(
		lockedSlug ? competitions.find((c) => c.slug === lockedSlug) : undefined,
	);
	const lockedNotFound = $derived(
		!loading && lockedSlug !== undefined && !lockedCompetition,
	);
...
<main class="min-w-0 w-full overflow-x-clip py-8">
	...
	{:else if guestSession && profileParticipants.length > 0}
		...
		/>
	{:else if lockedNotFound}
		<p class="text-sm text-destructive" role="alert">
			Lomba tidak ditemukan atau sudah ditutup. <a
				href="/daftar"
				class="underline">Lihat semua lomba</a
			>.
		</p>
	{:else}
		<RegistrationForm
			{competitions}
			onLogin={openLogin}
			lockedCompetitionId={lockedCompetition?.id}
		/>
	{/if}
</main>
