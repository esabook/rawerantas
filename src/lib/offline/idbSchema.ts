...
export function ensureAllStores(db: StoreLike): void {
	...
	if (!db.objectStoreNames.contains("demo_sponsors")) {
		db.createObjectStore("demo_sponsors", { keyPath: "id" });
	}
	if (!db.objectStoreNames.contains("demo_audit_logs")) {
		db.createObjectStore("demo_audit_logs", { keyPath: "id" });
	}
	if (!db.objectStoreNames.contains("demo_data_lock")) {
		db.createObjectStore("demo_data_lock", { keyPath: "id" });
	}
	if (!db.objectStoreNames.contains("demo_staff")) {
		db.createObjectStore("demo_staff", { keyPath: "id" });
	}
}
